...
mod log_compat {
    ...
    struct WriteLockingFlushHandler {
        acquired_write_lock: Arc<AtomicBool>,
    }
    ...
    impl FemtoHandlerTrait for WriteLockingFlushHandler {
        fn handle(&self, _record: FemtoLogRecord) -> Result<(), HandlerError> {
            Ok(())
        }

        fn flush(&self) -> bool {
            let acquired_write_lock = MANAGER.try_write().is_some();
            self.acquired_write_lock
                .store(acquired_write_lock, Ordering::SeqCst);
            false
        }
    ...
    #[test]
    #[serial]
    fn flush_all_handlers_releases_manager_before_handler_callbacks() {
        Python::attach(|py| {
            reset_manager();

            let acquired_write_lock = Arc::new(AtomicBool::new(false));
            let handler = Arc::new(WriteLockingFlushHandler {
                acquired_write_lock: acquired_write_lock.clone(),
            }) as Arc<dyn FemtoHandlerTrait>;
            let logger = match get_logger(py, "bridge.flush.reentrant") {
                Ok(logger) => logger,
                Err(error) => panic!("logger setup should succeed: {error}"),
            };
            logger.borrow(py).add_handler(handler);

            flush_all_handlers(py);

            assert!(
                acquired_write_lock.load(Ordering::SeqCst),
                "handler callbacks must run after the manager read lock is released",
            );
        });
    }
}
