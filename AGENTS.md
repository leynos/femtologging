...
- **Use consistent spelling and grammar.** Comments must use en-GB-oxendict
  ...
  references to external APIs. Markdown spelling is enforced by the `typos`
  checker through `make markdownlint`; see the
  [Developers Guide](docs/developers-guide.md#typos-spelling-checker) for the
  pinned version and how to regenerate `typos.toml`.
...
- [Python Code Style Guidelines](.rules/python-00.md) — Core Python style
  conventions
...
`make lint` runs four Python tiers in order: Ruff, Pylint (through the
PyPy-backed `pylint-pypy` shim), `df12-python-lints` with its companion
`ambrleaks` snapshot scanner, and a strict Skylos production dead-code gate.
See [Python linting](docs/developers-guide.md#python-linting) for the full
...
- Validate Markdown files using `make markdownlint`. This target also enforces
  en-GB-oxendict (Oxford "-ize") spelling with the `typos` checker, configured
  by `typos.toml` (generated by `scripts/generate_typos_config.py`).
