...
Clippy configuration and the compiled fixture live. Facts about the Makefile
are checked here instead, through the pinned `makeutil` parser, so the
repository keeps one Makefile parser rather than two. Facts about the workflows
live in `tests/test_env_access_policy_ci_contract.py`.
