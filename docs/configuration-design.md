...
the bounded-wait variant. The fluent stores the resolved `OverflowPolicy`,
keeping subsequent calls and the Rust build pipeline aligned. Direct
construction uses `HandlerOptions.policy`, which accepts the string forms
parsed by `file::parse_overflow_policy`: `"drop"`, `"block"`, or `"timeout:N"`
with a positive integer suffix. A bare `"timeout"` still raises the targeted
guidance error emitted by the parser.
