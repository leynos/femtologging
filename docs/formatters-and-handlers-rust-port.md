...
`FemtoFileHandler::with_capacity_policy()` allows specifying how to handle queue
overflows: drop new records (default), block indefinitely or wait for a timeout
before dropping. This lets applications choose between throughput and guaranteed
delivery.
